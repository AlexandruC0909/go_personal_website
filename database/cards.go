...
const getCardQuery = "SELECT c.id, c.name, c.content, c.position, c.type, c.parents_id "
...
func (s *DbConnection) CreateCard(card *types.Card) error {
	...
	(name, content, position, type, parents_id, created_at, updated_at)
...
func (s *DbConnection) UpdateCard(card *types.Card) error {
	updateQuery := `update cards set name = $1 , content = $2, position = $3, parents_id = $4, updated_at= $5 where id = $6 RETURNING id`
